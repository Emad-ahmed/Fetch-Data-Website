...
document.getElementById('tire4_three').addEventListener('click', latest_data);
...
function latest_data() {
    fetch('http://webapi19sa-1.course.tamk.cloud/v1/weather/wind_direction/71')
        ...
        <th class="table-active">Row Number</th>
            ...
            <th class="table-active">Wind Direction</th>
        </tr>`
            ...
            for (i = 0; i < data.length; i++){
                ...
                var total = sum += data[i].wind_direction
                var average15 = total / length;
            output +=
                    `<tr class="tableRow">
                    <td class="tableData">${i+1}</td>
                        ...
                        <td class="tableData">${data[i].wind_direction.slice(0,5)}</td>
  
                    </tr>`
            }
            document.getElementById('table17').innerHTML = output;
            document.getElementById('average15').innerHTML =average15;
        });
